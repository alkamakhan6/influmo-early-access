...
function offerCode(){ return 'EA-' + Date.now().toString(36).toUpperCase(); }
function offerExpires(days=90){ const d=new Date(); d.setDate(d.getDate()+days); return d.toISOString(); }
function getUTM(){
...
nextBtn?.addEventListener('click', () => {
  ...
  step1.style.display='none'; step2.style.display='grid'; roleEl.focus();
});
...
form?.addEventListener('submit', async (e) => {
  ...
  if (hp && hp.value) { joinBtn.disabled=false; joinBtn.textContent='Get early access'; return; }
  ...
  try {
    ...
    form.reset(); step2.style.display='none'; step1.style.display='grid';
    await refreshCount(); await refreshBreakdown();
  } catch (e) {
